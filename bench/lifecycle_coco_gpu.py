...
def validate_trial(value, spec, path, harness, corpus, fresh_sha, resume_input=None):
    ...
    require(
        value["script_sha256"] == harness["train_coco_gpu.py"] and value["harness_sources"] == harness,
        "trial harness drift",
    )
    ...
    for epoch in value["epochs"]:
        require(
            type(epoch["epoch"]) is int and epoch["images"] == 5000 and epoch["batches"] == 1250,
            "incomplete full-image epoch",
        )
        require(
            finite(epoch["seconds"], 1e-12)
            and finite(epoch["images_per_second"], 1e-12)
        ...
        require(
            len(trace) == 1250
            and all(len(row) == len(names) and all(finite(x, -math.inf) for x in row) for row in trace),
    ...
    require(
        {d["mode"] for d in value["datasets"]} == {"train", "val"} and len(value["datasets"]) == 2,
        "missing dataset construction",
    )
    require(
        all(
            d["images"] == 5000 and d["replaced_formats"] == int(spec["backend"] != "reference")
    ...
    require(
        [r["stage"] for r in lifecycle] == ["prepared"] + ["first-batch"] * len(expected_epochs),
    ...
    for index, observation in enumerate(lifecycle):
        require(
            type(observation["start_epoch"]) is int and observation["start_epoch"] == spec["start_epoch"],
            "wrong start epoch",
        )
        closed = spec["start_epoch"] > 2 if index == 0 else expected_epochs[index - 1] >= 2
        require(
            observation["formatter"] == observation["factory"] == wanted and observation["collator"] == collator,
            "formatter/factory/collator mismatch",
        )
        require(
            observation["rebuilt_from_initial"] is closed and observation["pin_memory"] is True,
            "wrong rebuild or pinning",
        )
        pids = observation["worker_pids"]
        require(
            len(pids) == len(set(pids)) == spec["workers"] and all(type(p) is int and p > 0 for p in pids),
            "invalid live worker IDs",
        )
        require(
            observation["worker_start_method"] in ("fork", "spawn", "forkserver")
            if spec["workers"]
            else observation["worker_start_method"] is None,
            "missing actual worker context",
        )
        require(
            observation["worker_start_method"] == lifecycle[0]["worker_start_method"],
            "worker context changed during trial",
        )
        require(observation["prefetch_factor"] == (4 if spec["workers"] else None), "wrong training prefetch setting")
        if index:
            require(
                observation["epoch"] == expected_epochs[index - 1] and observation["pinned_image"] is True,
                "wrong first-batch observation",
            )
        replacement = (index == 0 and spec["start_epoch"] > 2) or (index > 0 and expected_epochs[index - 1] == 2)
        if replacement:
            check_workers(
                observation["replaced_worker_pids"],
                observation["replaced_worker_exitcodes"],
                observation["replaced_workers_alive"],
                spec["workers"],
            )
            require(not set(pids) & set(observation["replaced_worker_pids"]), "worker PIDs reused at reset")
            if index:
                require(
                    observation["replaced_worker_pids"] == lifecycle[index - 1]["worker_pids"],
                    "replacement history mismatch",
                )
        else:
    ...
    require(
        value["worker_shutdown"]["train"]["worker_pids"] == lifecycle[-1]["worker_pids"],
        "shutdown observed another worker pool",
    )
    require(
        not set(value["worker_shutdown"]["train"]["worker_pids"]) & set(value["worker_shutdown"]["val"]["worker_pids"]),
    ...
    if spec["start_epoch"]:
        ...
        require(
            config["resume_from"] == resume_input["path"]
        ...
        require(
            restored["checkpoint_epoch"] == spec["start_epoch"] - 1
            and restored["optimizer_states"] > 0
            and restored["ema_updates"] > 0,
            "missing checkpoint optimizer/EMA state",
        )
        require(
            config["checkpoint_epochs"] == [] and value["checkpoints"] == [],
            "resumed trial requested new checkpoint cohort",
        )
    else:
        require(
            value["resume_input"] is None and config["checkpoint_epochs"] == [1, 2], "wrong fresh checkpoint protocol"
        )
        ...
        for checkpoint in value["checkpoints"]:
            ...
            require(
                ...
                and file_sha(checkpoint_path) == checkpoint["sha256"],
    ...
    require(
        samples
        and all(
            finite(s["time"], 1) and type(s["family_rss_bytes"]) is int and s["family_rss_bytes"] > 0 for s in samples
        ),
        "missing resource samples",
    )
    require(
        value["sampled_peak_family_rss_bytes"] == max(s["family_rss_bytes"] for s in samples), "wrong family RSS peak"
    )
    compact = {k: v for k, v in value.items() if k not in ("epochs", "resource_samples")}
    compact["epochs"] = epochs
    return compact, {
        "initial_model": value["initial_state_sha256"],
        "final_model": value["final_state_sha256"],
...
def main():
    ...
    require(
        not out.exists() and not directory.exists() and not temporary.exists(),
        "retain previous results; choose a new output",
    )
    ...
    report = {
        ...
        "planned_epochs": sum(4 - s["start_epoch"] for s in plan),
        "plan": plan,
        "records": [],
        "cohorts": {},
    }
    ...
    try:
                ...
                try:
                    ...
                    if process.poll() is None:
                        ...
                        try:
                            ...
                            process.wait()
                    record.update(
                        returncode=process.returncode, finished_at_ns=time.time_ns(), log_sha256=file_sha(log)
                    )
                    if path.exists():
        ...
        report.update(
            ...
            full_protocol_complete=report["full_protocol_requested"],
