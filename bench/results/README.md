...
- `rfdetr-loader-linux-v1.json`: RF-DETR segmentation data loading (`bench/rfdetr_loader.py`) on the i5-10400: RF-DETR develop `2398ce3c` (1.11.0.dev0), torch 2.10 CPU, COCO val2017, default training transforms (`make_coco_transforms_square_div_64`, multi-scale, resolution 560). 400 samples checked byte for byte under the same seeds, then three alternating single-process rounds with per-stage timers and an 8-worker DataLoader for each pipeline.
- `rfdetr-loader-linux-v2.json`: the same measurement after the kernel's walk and sweep optimizations (`44053c1`); materialize 0.34 → 0.23 ms per sample.
- `workers-sweep-linux-v1/`: DataLoader throughput against `num_workers` (0, 1, 2, 4, 6, 8, 12) for both frameworks (`bench/workers_sweep.py`), same host: `rfdetr-workers.json` (`bench/rfdetr_workers.py`, reference vs accelerated, fresh process per run, two rounds, steady-state after the first batch) and `ultralytics-workers-*.json` (`bench/coco_epoch.py` on val2017 per worker count, reference vs maskops, two rounds, 16 verification batches) with the per-worker summary in `ultralytics-workers.json`.
- `rfdetr-verify-linux-v1.json`: byte parity of the RF-DETR adapter over the whole val2017 split (5,000 images, two seeds each) plus 2,000 random-polygon fuzz cases of the kernel against pycocotools + torchvision (`bench/verify_rfdetr.py`), same host and versions.
