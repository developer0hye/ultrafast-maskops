...
CASES = [{"n": n, "h": 640, "w": 640, "ratio": 4, "vertices": 16}
         for n in (0, 1, 5, 20, 100, 255, 256, 500)] + [
    {"n": 100, "h": 640, "w": 640, "ratio": 1, "vertices": 16}]


def fixture(case):
    rng = np.random.default_rng(20260912 + case["n"])
    centers = rng.uniform(.1, .9, (case["n"], 1, 2))
    angles = np.linspace(0, 2 * np.pi, case["vertices"], endpoint=False)
    radii = rng.uniform(.015, .2, (case["n"], 1, 1))
    return ((centers + radii * np.stack([np.cos(angles), np.sin(angles)], -1))
            * np.array([case["w"], case["h"]])).astype(np.float32)
...
def environment():
    return {"platform": platform.platform(), "machine": platform.machine(), "python": sys.version,
            ...
            "available_ram_bytes": psutil.virtual_memory().available, "loadavg": os.getloadavg(),
            "numpy": np.__version__, "cv2": cv2.__version__, "native": native.backend_info(),
            "cv2_build": cv2.getBuildInformation(), "native_build": native._native.opencv_build_info()}
...
def worker(index, backend, expected, samples):
    ...
    if backend == "reference":
        ...
        fn = lambda: native.Rasterizer().overlap(shape, native.PackedPolygons.from_segments(segments), r, mode="bounded")
    ...
    return {"case": index, "backend": backend, "samples_ns": raw,
            "median_ns": float(np.median(raw)), "p95_ns": float(np.percentile(raw, 95)),
            "peak_rss_bytes": usage.ru_maxrss * (1 if sys.platform == "darwin" else 1024),
            "user_s": usage.ru_utime, "system_s": usage.ru_stime,
            "input_sha256": hashlib.sha256(segments.tobytes()).hexdigest(), "output_hashes": expected,
            "parity": True, "available_ram_bytes": psutil.virtual_memory().available,
...
def main():
    ...
    cv2.setNumThreads(0)
    report = {"environment": environment(), "cases": CASES, "rounds": args.rounds, "samples": args.samples,
              ...
              "os_cache": "uncontrolled", "warmups": 10, "results": []}
    ...
    for round_ in range(args.rounds):
        for index in range(len(CASES)):
            ...
            for backend in backends:
                output = subprocess.check_output([sys.executable, __file__, "--worker", str(index),
                    "--backend", backend, "--samples", str(args.samples), "--expected", json.dumps(expected[index])], text=True)
    ...
    for index in range(len(CASES)):
        ...
        for backend in ["wrapper", "bounded"]:
            ...
            summaries.append({"case": index, "backend": backend, "speedup_median": float(np.median(ratios)),
