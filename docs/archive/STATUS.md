...
- Independent audits of immutable eight-, seventeen-, twenty-seven- and thirty-six-trial checkpoints verified raw/compact hashes, commands, runtime/configuration, complete loss traces and memory accounting. The latest checkpoint contains 72 full epochs and twelve complete backend groups (workers 0/2/8, first four repetitions), all with identical loss traces and final models within each group. The auditor's 12 synthetic corruption/completion tests passed on M2; the complete real-series summary audit also passed, as recorded in GPU_RESULTS.md.
