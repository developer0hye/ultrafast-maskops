"""Test eager shared-memory preparation against reference worker teardown.

Diagnostic only: this wrapper still adds a per-batch copy and is not a proposed
optimized collation implementation. Both new libraries remain unimported.
"""
...
def shared_reference_collate(batch):
...
def equal_batch(expected, actual):
    assert expected.keys() == actual.keys()
    for key in expected:
        assert type(expected[key]) is type(actual[key]), key
...
def main():
    ...
    parser.add_argument("--shared-batch", action="store_true")
    ...
    with tempfile.TemporaryDirectory(prefix="reference-reset-") as temporary:
        ...
        try:
            ...
            loader = InfiniteDataLoader(
                ...
                collate_fn=shared_reference_collate if args.shared_batch else YOLODataset.collate_fn,
            ...
            for i, batch in enumerate(batches):
                equal_batch(YOLODataset.collate_fn([dataset[i * 2], dataset[i * 2 + 1]]), batch)
            record["post_reset_batches_match_reference"] = True
            record["events"].append("consumed-after-reset")
            restarted = list(loader.iterator._workers)
            loader.close()
            record["new_worker_exitcodes"] = [w.exitcode for w in restarted]
            assert all(not w.is_alive() and w.exitcode == 0 for w in restarted)
            record["events"].append("clean-final-close")
